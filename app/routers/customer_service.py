...
from app.dependencies import get_support_admin_service, get_user_repository, require_role
...
def list_customer_service(
    ...
    return SupportListResponse(
        items=[SupportSummary(**support_list_payload(item, user_labels=user_labels)) for item in items]
    )
...
def get_customer_service(
    ...
    user_labels = closed_by_user_labels([ticket], users)
    return SupportDetail(**support_detail_payload(ticket, conversation, user_labels=user_labels))
...
def update_customer_service(
    ...
    ticket = service.update_status(ticket_id, status_value, closed_by=closed_by)
    if ticket is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Support request not found")
    detail = service.get_ticket_detail(ticket_id)
    conversation = detail[1] if detail else None
    user_labels = closed_by_user_labels([ticket], users)
    if ticket.closed_by and ticket.closed_by not in user_labels:
        user_labels[ticket.closed_by] = current_user.email
    return SupportDetail(**support_detail_payload(ticket, conversation, user_labels=user_labels))
