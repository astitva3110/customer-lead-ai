...
def _actionable_lead(message: str, product: str, knowledge: bool) -> bool:
    if _matches(message, LEAD_PATTERNS):
        return True
    if knowledge:
        return False
...
class ChatRouter:
    ...
    def route(self, state: ConversationState) -> ConversationState:
        ...
        if current_trace():
            method = "llm" if state.trace.get("turn_understanding_llm") else "deterministic"
    ...
    def _understand(self, state: ConversationState) -> TurnUnderstanding:
        ...
        lead_phrase = _matches(message, LEAD_PATTERNS) or _actionable_lead(message, product, False)
        ...
        if PHONE_LIKE.search(message) and state.mode in {ChatMode.LEAD, ChatMode.SUPPORT} and not knowledge_phrase:
            providing = True
        mixed = knowledge_phrase and (lead_phrase or support_phrase or contact)
        ...
        return TurnUnderstanding(
            turn_intent=intent,
            needs_rag=needs_rag,
            lead_intent=lead_phrase or contact,
    ...
    def _apply_understanding(
        ...
        *,
        ...
        if updates.get("phone") and not state.phone:
            parsed = extract_phone_from_text(updates["phone"] or "")
            if parsed:
        ...
        if intent in {TurnIntent.LEAD_INTENT, TurnIntent.MIXED} and understanding.lead_intent and not understanding.support_intent:
            ...
            state.lead_status = state.lead_status if state.lead_status != LeadStatus.IDLE else LeadStatus.COLLECTING
            state.lead_workflow = (
                state.lead_workflow
        ...
        if understanding.needs_rag or intent == TurnIntent.KNOWLEDGE:
            if state.conversation_goal in {ConversationGoal.LEAD, ConversationGoal.SUPPORT}:
                ...
                goal = ""
            if goal == ChatMode.SUPPORT and state.ticket_status != TicketStatus.CREATED:
                state.return_mode = ChatMode.SUPPORT
            elif goal == ChatMode.LEAD and state.lead_status != LeadStatus.CREATED:
                state.return_mode = ChatMode.LEAD
            if not state.conversation_goal or state.conversation_goal == ConversationGoal.NONE:
