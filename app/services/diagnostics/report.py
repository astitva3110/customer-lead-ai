...
def format_chat_trace_text(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "SEMANTIC ROUTER",
        f"executed: {(payload.get('semantic_router') or {}).get('executed')}",
        f"used: {(payload.get('semantic_router') or {}).get('used')}",
        f"route: {(payload.get('semantic_router') or {}).get('route')}",
        f"canonical_query: {(payload.get('semantic_router') or {}).get('canonical_query')}",
        f"product: {(payload.get('semantic_router') or {}).get('product')}",
        f"sales_interest: {(payload.get('semantic_router') or {}).get('sales_interest')}",
        f"diverge: {(payload.get('semantic_router') or {}).get('diverge')}",
        f"model: {(payload.get('semantic_router') or {}).get('model')}",
        f"confidence: {(payload.get('semantic_router') or {}).get('confidence')}",
        f"fallback_reason: {(payload.get('semantic_router') or {}).get('fallback_reason')}",
        f"input_tokens: {(payload.get('semantic_router') or {}).get('input_tokens')}",
        f"output_tokens: {(payload.get('semantic_router') or {}).get('output_tokens')}",
        f"total_tokens: {(payload.get('semantic_router') or {}).get('total_tokens')}",
        f"latency_ms: {(payload.get('semantic_router') or {}).get('latency_ms')}",
        "",
        "RETRIEVAL",
